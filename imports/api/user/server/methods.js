...
Meteor.methods({
   ...
   * @returns {Promise<*>}
   */
  async method__user_password_reset(data) {
    schema__password_reset.validate(data)

    let url = object_value(Meteor, 'settings.private.API.password_reset')
    if (!url) {
      throw new Meteor.Error('no-request-url')
    }
